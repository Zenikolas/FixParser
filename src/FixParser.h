...
public:
    ...
    // callback which is intended to be called by DataReader on each tick's 'line'
...
private:
    ...
    bool mandatory_tag_occurred(std::string_view &mandatory_value, int tag, const std::string_view &value,
                                const std::string_view &msg_type);
